...
var DecisionTask = function (decisionTask, context) {
...
DecisionTask.prototype = {
   ...
   stop: function (stopAttributes, swfAttributes) {
     ...
     */
      ...
      for (i = 0; i < this._events.length; i++) {
        ...
        if (evt.eventType === "LambdaFunctionFailed") {
          if (this.lambdaIdFor(evt.lambdaFunctionFailedEventAttributes.scheduledEventId) === lambdaId) {
            ...
            if (!details) {
              console.log("WARNING Received a lambdaFunctionFailed without details or reason");
              console.log(evt);
            ...
            try {
              ...
              console.log("WARNING Received a lambdaFunctionFailed with non JSON failure details");
              console.log(evt);
              return details;
            }
          }
     ...
     // Call a lambda function via another intermediary lambda function to propagate the alias used on the decider
     lambda_with_alias: function (scheduleAttributes, intermediaryLambdaFunctionName, alias) {
       // Get the alias used on the decider
       if (!alias) {
         ...
         if (!match || !match[2]) {
           this.addDecision({
             ...
             "failWorkflowExecutionDecisionAttributes": {
               reason: "lambda_with_alias could not read the alias from the invoked function arn",
               details: scheduleAttributes
             }
           });
         }
         alias = match[2];
       ...
       var intermediaryLambda = intermediaryLambdaFunctionName || "callLambdaWithAlias";

       var sla = {
         id: scheduleAttributes.id,
         name: intermediaryLambda,
         startToCloseTimeout: scheduleAttributes.startToCloseTimeout,
         input: {
           input: scheduleAttributes.input,
           functionName: scheduleAttributes.name,
           alias: alias,
           startToCloseTimeout: scheduleAttributes.startToCloseTimeout,
         }
       };
       console.log("about to schedule "+scheduleAttributes.id+" to call lambda "+scheduleAttributes.name+" with alias "+alias+" via intermediary lambda "+intermediaryLambda);
       return this.lambda(sla);
     },
     ...
     lambda: function (scheduleAttributes) {
       ...
       var that = this;
         return function(cb) {
              ...
              * @see http://docs.aws.amazon.com/amazonswf/latest/awsrbflowguide/programming-with-retry.html
              * It would be cool to also allow to specify wether to retry or not based in some conditions
              * (ex: retry only activities that timed out, but not other kind of failures)
              */
             if( that.is_lambda_scheduled(scheduleAttributes.id) ) {
                 if( that.has_lambda_completed(scheduleAttributes.id) ) {
                     ...
                     console.log("waiting for "+scheduleAttributes.id+" lambda to complete.");
                     that.wait();
                 }
             }
             else {
                 console.log("scheduling "+scheduleAttributes.id);
        ...
        return function (cb) {
                if(that.childworkflow_failed(startAttributes.name)) {
                  cb(that.childworkflow_failure_details(startAttributes.name));
                    ...
                    console.log("starting childworkflow "+startAttributes.name);
  ...
  _has_event_with_attribute_value: function(attributeKey, attributeValue, eventType) {
   ...
   childworkflow_scheduled: function(control) {
      ...
      console.log('childworkflow_scheduled('+control+') ? => ', ret);
      return ret;
   },
   ...
   childworkflow_completed: function(control) {
      ...
      console.log('childworkflow_completed('+control+') ? => ', ret);
      return ret;
   },
   ...
   childworkflow_failed: function(control) {
      ...
      var ret = this._events.some(function (evt) {
         if (evt.eventType === "StartChildWorkflowExecutionFailed") {
            ...
            if (!initiatedEventId) {
              console.log("FATAL ERROR: Child workflow failed - " + evt.startChildWorkflowExecutionFailedEventAttributes.cause)
              console.log(evt);
              return true;
            }
      ...
      console.log('childworkflow_failed('+control+') ? => ', ret);
      return ret;
   },
   ...
   has_lambda_timedout: function (lambdaId) {
      ...
      if (!ret) {
         ...
         *
         * lambda_timedout_details is also patched to return an error message
         * if either it finds a LambdaFunctionTimedOut
         * or a LambdaFunctionFailed event with a message containing some specific timeout details
         */ 
        ret = !!this.lambda_timedout_details(lambdaId);
      }

      console.log('has_lambda_timedout('+lambdaId+') ? => ', ret);
      return ret;
   },

   is_lambda_started: function (lambdaId) {
      var ret = this._has_eventType_for_lambdaId(lambdaId, "LambdaFunctionStarted");
      console.log('has_lambda_completed('+lambdaId+') ? => ', ret);
      return ret;
   },
